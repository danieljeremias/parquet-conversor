...
public class ParquetTransformer {
    ...
    public void transform(String bucket, String key) {
        try {
            ...
            try {
                ...
                LOGGER.info("   Reading first line of {}", source);
                if (first == null) {
                    return;
                }
            ...
            LOGGER.error("Failed to transform file: {}", key, e);
    ...
    private void cleanupTempFiles(Path source, Path target) {
        try {
            if (source != null && Files.exists(source)) {
                Files.delete(source);
                LOGGER.info("   Deleted temp source file: {}", source);
            }
        } catch (Exception e) {
        ...
        try {
            if (target != null && Files.exists(target)) {
                Files.delete(target);
                LOGGER.info("   Deleted temp target file: {}", target);
            }
        } catch (Exception e) {
    ...
    private void processRecord(GenericRecord record, Schema newSchema, ParquetWriter<GenericRecord> writer) throws Exception {
        LOGGER.info("   Writing new record {}", record.toString());
        GenericRecord newRecord = new GenericData.Record(newSchema);
        for (Schema.Field field : newSchema.getFields()) {
            Object value = record.get(field.name());
            if ("transaction_id".equals(field.name())) {
                if (value != null) {
                    LOGGER.info("      Transaction ID is not null");
